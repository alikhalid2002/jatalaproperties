...
const SearchResults = ({ query, data, onNavigate }) => {
  ...
  return (
    <div className="flex-1 space-y-8 p-4"><h2 className="text-3xl font-black">Search: "{query}" ({total} found)</h2>
...
const SearchCard = ({ title, sub, type, icon, color, onClick }) => (
  <button onClick={onClick} className="bg-slate-800/40 p-6 rounded-3xl border border-slate-700/50 flex items-center gap-4 text-left group">
    ...
    <div><p className={`text-[9px] font-black uppercase text-${color}-500`}>{type}</p><h3 className="font-black text-white">{title}</h3><p className="text-[10px] text-indigo-200/80 uppercase">{sub}</p></div>
  </button>
);
